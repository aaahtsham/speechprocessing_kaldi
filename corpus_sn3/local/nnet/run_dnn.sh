...
steps/nnet/make_fmllr_feats.sh --nj 3 --cmd $train_cmd \
...
steps/decode_nnet.sh --nj 3 --cmd $decode_cmd --config conf/decode_dnn.config \
  --nnet $dnnDir/final.nnet --acwt 0.08 $baseDir/graph data/test_fmllr $dnnDir/decode || exit 1;

steps/nnet/align.sh --nj 3 --cmd $train_cmd data/train_fmllr data/lang \
  $dnnDir $align_dnnDir || exit 1;

steps/nnet/make_denlats.sh --nj 3 --cmd $train_cmd --config conf/decode_dnn.config --acwt 0.1 \
...
for n in 6 5 4 3 2 1; do
  steps/decode_nnet.sh --nj 3 --cmd $train_cmd --config conf/decode_dnn.config \
